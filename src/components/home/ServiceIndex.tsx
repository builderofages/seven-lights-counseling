    ...
    <section id="services" className="tint tint-4 relative bg-surface py-section">
      <div className="shell">
            ...
            {services.map((s, i) => (
              <Reveal key={s.slug} delay={Math.min(i * 40, 220)}>
                <Link
                  ...
                  onFocus={() => setActive(i)}
                  className="group relative flex items-center gap-5 border-b border-line/[0.13] py-6 sm:gap-7"
                >
                  <span
                    className={cn(
                      "w-8 shrink-0 font-sans text-[0.6875rem] font-semibold tracking-[0.16em] transition-colors duration-500",
                      active === i ? "text-accent" : "text-accent/45",
                    )}
                  >
                    {s.index}
                  </span>
                  ...
                  <span className="min-w-0 flex-1">
                    <span
                      className={cn(
                        ...
                        active === i ? "text-fg" : "text-fg/70",
                      )}
                    >
                      {s.title}
                    </span>
                    <span className="mt-1.5 block font-sans text-[0.8125rem] uppercase tracking-[0.13em] text-fgm/45">
                      {s.eyebrow}
                    </span>
                  </span>
          ...
          <div className="hidden lg:col-span-5 lg:block">
            <div className="sticky top-28">
              <div className="relative aspect-[4/5] overflow-hidden rounded-2xl bg-surface-2">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-scrim/45 via-transparent to-transparent" />
                <div className="absolute inset-x-0 bottom-0 p-7">
                  <p className="font-sans text-[0.625rem] font-semibold uppercase tracking-[0.2em] text-onc/60">
                    {services[active].index} · {services[active].duration}
                  </p>
                  <p className="mt-2.5 font-display text-[1.5rem] font-light leading-tight text-onc">
                    {services[active].title}
                  </p>
                </div>
              </div>

              <p className="mt-6 max-w-[42ch] font-sans text-[0.9375rem] leading-[1.7] text-fgm/70">
        ...
        <Reveal delay={100} className="mt-14 flex flex-wrap items-center gap-4">
          ...
          <p className="font-sans text-[0.875rem] text-fgm/55">
