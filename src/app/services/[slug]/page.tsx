        ...
        <div className="shell grid gap-12 lg:grid-cols-12">
          ...
          <ul className="lg:col-span-7 lg:col-start-6">
            {service.signals.map((sig, i) => (
              <Reveal key={sig} as="li" delay={i * 60} className="border-b border-line/[0.13] first:border-t">
                <p className="flex items-start gap-5 py-6 font-display text-[clamp(1.1rem,2vw,1.5rem)] font-light leading-[1.35] tracking-[-0.02em] text-fg/85">
                  <span className="mt-[0.6em] block h-px w-6 shrink-0 bg-accent/70" />
                  {sig}
                </p>
              </Reveal>
            ))}
      ...
      <section className="bg-surface-2 py-section">
        <div className="shell grid gap-14 lg:grid-cols-12">
          ...
          <div className="lg:col-span-6 lg:col-start-7">
            ...
            <Reveal delay={100}>
              <div className="mt-12 grid gap-x-8 gap-y-6 border-t border-line/[0.13] pt-8 sm:grid-cols-2">
                <div>
                  <p className="eyebrow">Session shape</p>
                  <p className="mt-2 font-sans text-[0.9375rem] text-fgm/80">{service.duration}</p>
                </div>
                <div>
                  <p className="eyebrow">Approaches drawn on</p>
                  <p className="mt-2 font-sans text-[0.9375rem] leading-relaxed text-fgm/80">
        ...
        <div className="shell relative">
          <div className="grid gap-8 lg:grid-cols-12 lg:items-end">
            <div className="lg:col-span-7">
              <Reveal>
                <p className="eyebrow text-accent">Arc of treatment</p>
              </Reveal>
              <SplitLines
                as="h2"
          ...
          <ol className="mt-16 grid gap-px overflow-hidden rounded-2xl bg-surface/12 md:grid-cols-2 xl:grid-cols-4">
            {service.work.map((w, i) => (
              <Reveal
                ...
                as="li"
                delay={i * 90}
                className="group relative bg-contrast p-8 transition-colors duration-700 hover:bg-contrast-2"
              >
                ...
                <p className="mt-4 font-sans text-[0.9375rem] leading-[1.7] text-onc/60">{w.text}</p>
              </Reveal>
            ))}
          </ol>

          {/* lights engaged */}
          <div className="mt-20 grid gap-10 border-t border-onc/12 pt-12 lg:grid-cols-12">
            <Reveal className="lg:col-span-4">
              <p className="eyebrow text-accent">Lights engaged</p>
              <p className="mt-5 max-w-[34ch] font-sans text-[0.9375rem] leading-relaxed text-onc/55">
                Where this work sits on the map. Most people never think about it —
                it simply keeps the treatment coherent.
              </p>
              <Link href="/approach" className="link-sweep mt-6 inline-block font-sans text-[0.875rem] text-onc/70">
                See the full method
              </Link>
            </Reveal>
            ...
            <div className="grid gap-4 sm:grid-cols-3 lg:col-span-7 lg:col-start-6">
              {engaged.map((l, i) => (
                <Reveal key={l.n} delay={i * 90} className="rounded-2xl border border-onc/12 p-6">
                  ...
                  <p className="mt-1 font-sans text-[0.75rem] uppercase tracking-[0.14em] text-onc/35">
                    {l.latin}
                  </p>
                  <p className="mt-4 font-sans text-[0.875rem] leading-relaxed text-onc/55">
                    {l.theme}
                  </p>
                </Reveal>
              ))}
              ...
              <Reveal key={r.slug} delay={i * 90}>
                <Link href={`/services/${r.slug}`} className="group block">
                  <span className="relative block aspect-[4/3] overflow-hidden rounded-2xl">
                    ...
                    />
                    <span className="absolute inset-0 bg-scrim/10 transition-opacity duration-700 group-hover:opacity-0" />
                  </span>
                  <span className="mt-5 flex items-baseline gap-3">
                    <span className="font-sans text-[0.625rem] font-semibold tracking-[0.16em] text-accent/70">
                      {r.index}
                    </span>
                    <span className="font-display text-[1.3rem] font-light leading-tight text-fg">
                      {r.title}
                    </span>
                  </span>
                  <span className="mt-2 block font-sans text-[0.875rem] leading-relaxed text-fgm/55">
                    {r.eyebrow}
                  </span>
                </Link>
              </Reveal>
